...
describe("guard middleware", () => {
	...
	it("uses custom name for authorize/revoke", () => {
		...
		expect(getInternals(server).isToolVisible("admin_panel", "s1")).toBe(false);

		const session = getInternals(server).createSessionAPI("s1");
		session.authorize("admin");

		expect(getInternals(server).isToolVisible("admin_panel", "s1")).toBe(true);
	});
	...
	it("supports multiple independent guard scopes", () => {
		...
		expect(getInternals(server).isToolVisible("weather", "s1")).toBe(false);
		expect(getInternals(server).isToolVisible("admin_panel", "s1")).toBe(false);

		// Authorize guard scope only
		session.authorize("guard");
		expect(getInternals(server).isToolVisible("weather", "s1")).toBe(true);
		expect(getInternals(server).isToolVisible("admin_panel", "s1")).toBe(false);
