...
export function githubOAuth(options: GitHubOAuthOptions): ToolMiddleware {
	const scopes = options.scopes ?? [];

	const opts: Parameters<typeof oauth>[0] = {
		name: options.name ?? "github-oauth",
...
export interface HandleGitHubCallbackOptions {
	clientId: string;
	clientSecret: string;
	/** Session key for user data. Default: "user" */
	sessionKey?: string;
...
export async function handleGitHubCallback(
	...
	options: HandleGitHubCallbackOptions,
