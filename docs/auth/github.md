...
|--------|------|---------|-------------|
| `name` | `string` | `"github-oauth"` | Middleware name |
...
::: tip Under the hood
`githubOAuth()` wraps `oauth()` which wraps `urlAction()`. When a protected tool
...
callback URL. `handleGitHubCallback()` exchanges the authorization code for an
